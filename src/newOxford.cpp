...
EXP double Oxford::value(const actions::Actions &J, const double Jcritr) const
{
	double hJ=h(J);
	double gJ=hJ;
...
Oxford::Oxford(const OxfordParam &inparams, const potential::Interpolator& infreq) :  par(inparams), freq(infreq)
{
	norm=1;
